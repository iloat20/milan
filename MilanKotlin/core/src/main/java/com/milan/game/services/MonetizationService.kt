...
class MonetizationService(
    ...
    /** 获取变现数据（契约名 [MonetizationApi.getMonetizationData]）。 */
    override fun getMonetizationData(): MonetizationSaveData {
    ...
    override suspend fun activateMonthlyCard(cost: Int): WriteOutcome {
        if (cost <= 0) return WriteOutcome.Rejected // R5-I11：负数会让余额校验失效并反向加钱
        // R7-P1：余额校验入锁
        return core.withWriteLock {
            val data = getMonetizationData()
            if (core.saveData.hardCurrency < cost) return@withWriteLock WriteOutcome.Rejected
    ...
    /** 领取月卡每日奖励。 */
    @Deprecated("P2-11: 月卡UI零调用", level = DeprecationLevel.WARNING)
    override suspend fun claimMonthlyCardReward(): WriteOutcome {
        // R7-P1：active/daysLeft/今日已领校验入锁
        return core.withWriteLock {
            val data = getMonetizationData()
            if (!data.monthlyCardActive) return@withWriteLock WriteOutcome.Rejected
            if (data.monthlyCardDaysLeft <= 0) return@withWriteLock WriteOutcome.Rejected

            val now = System.currentTimeMillis()
            val oneDayMs = 24 * 60 * 60 * 1000L
            if (now - data.monthlyCardLastClaim < oneDayMs) return@withWriteLock WriteOutcome.Rejected
    ...
    /** 月卡剩余天数。 */
    @Deprecated("P2-11: 月卡UI零调用", level = DeprecationLevel.WARNING)
    override fun getMonthlyCardDaysLeft(): Int = getMonetizationData().monthlyCardDaysLeft

    // ─────────────────── 通行证 ───────────────────

    /** 购买豪华通行证。 */
    override suspend fun purchaseBattlePass(cost: Int): WriteOutcome {
        if (cost <= 0) return WriteOutcome.Rejected // R5-I11：负数会让余额校验失效并反向加钱
        // R7-P1：余额/premium 校验入锁
        return core.withWriteLock {
            val data = getMonetizationData()
            if (core.saveData.hardCurrency < cost) return@withWriteLock WriteOutcome.Rejected
            if (data.battlePassPremium) return@withWriteLock WriteOutcome.Rejected
    ...
    /** 增加通行证经验（契约入口 [MonetizationApi.grantBattlePassExp]：独立操作，广播快照）。 */
    override suspend fun grantBattlePassExp(amount: Int): WriteOutcome =
        grantBattlePassExpInternal(amount, broadcast = true)
     ...
     *   作为独立操作调用时才传 `true`。
     */
    suspend fun grantBattlePassExpInternal(amount: Int, broadcast: Boolean): WriteOutcome {
        val data = getMonetizationData()
    ...
    /** 获取通行证定义奖励列表。 */
    override fun getBattlePassRewards(): List<BattlePassReward> {
    ...
    override suspend fun charge(tierId: String, hardCurrency: Int, costCents: Int): WriteOutcome {
        ...
        return core.withWriteLock {
            ...
            // R7-P1：hardCurrency*2 可 Int 溢出 → 首充白扣标记零到账
            val actualHC = if (isDouble) {
                val doubled = hardCurrency.toLong() * 2L
                if (doubled > Int.MAX_VALUE) Int.MAX_VALUE else doubled.toInt()
            } else {
                hardCurrency
            }
    ...
    override suspend fun claimChargeMilestone(amountCents: Int): WriteOutcome {
        ...
        // R7-P1：total/claimed 校验入锁
        return core.withWriteLock {
            val data = getMonetizationData()
            if (data.totalChargeAmount < amountCents) return@withWriteLock WriteOutcome.Rejected
            if (data.claimedChargeMilestones.contains(amountCents)) return@withWriteLock WriteOutcome.Rejected
    ...
    override fun getChargeTiers(): List<ChargeTier> = listOf(
    ...
    /** 获取累计充值里程碑。 */
    @Deprecated("P2-11: 充值里程碑UI零调用", level = DeprecationLevel.WARNING)
    override fun getChargeMilestones(): List<ChargeMilestone> = listOf(
