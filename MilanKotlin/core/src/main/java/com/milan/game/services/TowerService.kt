...
import com.milan.game.domain.deck.Deck
import com.milan.game.domain.deck.DeckContext
import com.milan.game.domain.deck.DeckRule
import com.milan.game.domain.deck.MaxSizeRule
import com.milan.game.domain.deck.OwnershipRule
 ...
 */
class TowerService(private val core: ServiceCore) : TowerApi {
    ...
    override fun getFormation(): List<String> = saveData.getFormationIds()

    /**
     * 出战编队的约束集合（2026-09-08 P0-2）：约束在此**集中声明**，供两处写入口共用。
     *
     * 此前「容量上限 / 必须已拥有」以 if 形式在 [setFormation] 与 [toggleFormation] 各写一遍；
     * 新增赛季禁用表、稀有度上限等约束时只需在此追加一个 [DeckRule]，[Deck] 本体无需改动。
     * 规则顺序即校验优先级：[MaxSizeRule] 在前，命中即短路返回。
     */
    private val formationRules: List<DeckRule> =
        listOf(MaxSizeRule(SaveData.MAX_FORMATION_SIZE), OwnershipRule)

    /** 构造规则校验上下文（读 saveData，须在 writeMutex 临界区内调用）。 */
    private fun formationContext(): DeckContext =
        DeckContext(saveData.ownedCharacters.filterNotNull().mapTo(HashSet()) { it.characterId })

    /**
     * 设置出战编队。
     *
     * 校验（任一不过返回 [WriteOutcome.Rejected]，不做任何变更）见 [formationRules]。
     * 允许空列表 = 清空编队。
     *
     * 输入规范化：[Deck.of] 会 trim 后丢弃空白 id 并去重，因此 `["a", ""]` 等价于 `["a"]`
     * （旧实现会因此整体拒绝）。空白 id 不构成合法卡牌，宽容丢弃优于让整个操作失败。
     *
     * 事务范式：落盘失败回滚本次改动、不广播事件。
     */
    override suspend fun setFormation(characterIds: List<String>): WriteOutcome = core.withWriteLock {
        val deck = Deck.of(characterIds)
        if (deck.validate(formationRules, formationContext()) != null) {
        ...
        core.transactionLocked(
            tag = "formation",
            mutate = { saveData.formation = deck.slots },
     ...
     */
    override suspend fun toggleFormation(characterId: String): WriteOutcome = core.withWriteLock {
        if (characterId.isBlank()) return@withWriteLock WriteOutcome.Rejected
        val context = formationContext()
        if (characterId !in context.ownedIds) return@withWriteLock WriteOutcome.Rejected

        // 走 Deck 领域模型：入队/出队 = withToggled，容量与拥有权约束复用 formationRules，
        // 不再在本方法内重复一遍 if 校验（P0-2）。
        val current = Deck.of(saveData.getFormationIds())
        val next = current.withToggled(characterId)
        if (next == current) return@withWriteLock WriteOutcome.Rejected
        if (next.validate(formationRules, context) != null) return@withWriteLock WriteOutcome.Rejected

        val original = saveData.formation
        core.transactionLocked(
            tag = "formation.toggle",
            mutate = { saveData.formation = next.slots },
            rollback = { saveData.formation = original },
            onCommit = { core.refreshSnapshot() },
        )
    }
    ...
    override suspend fun runTowerFloor(floor: Int): TowerOutcome = core.withWriteLock {
        ...
        val origEquipments = saveData.ownedEquipments.toList()
        ...
        val outcome = core.transactionLocked(
            ...
            mutate = {
                ...
                // C3（2026-09-09）：刷新最高层且层数为 10 的倍数 → 发放一件装备（临界区内直接 append）
                if (newBest != null) {
                    val drop = core.rollTowerEquipmentDrop(newBest)
                    if (drop != null) {
                        saveData.ownedEquipments = saveData.ownedEquipments + listOf(drop)
                    }
                }
            },
            rollback = {
     ...
     * @return 新的战斗状态
     */
    override fun executeStrategicAction(
     ...
     * @return 新的战斗状态
     */
    override fun executeStrategicEnemyTurn(
        state: com.milan.game.domain.battle.BattleState,
    ): com.milan.game.domain.battle.BattleState {
     ...
     * @return 新的战斗状态
     */
    override fun updateStrategicTurnState(
        state: com.milan.game.domain.battle.BattleState,
    ): com.milan.game.domain.battle.BattleState {
     ...
     * @return 战斗阶段
     */
    override fun checkStrategicBattleResult(
        state: com.milan.game.domain.battle.BattleState,
     ...
     * @return 可用行动列表
     */
    override fun getStrategicAvailableActions(
        state: com.milan.game.domain.battle.BattleState,
        actorIndex: Int,
    ...
    /**
     * 扫荡已通关的无尽之塔楼层（跳过战斗，直接发放奖励）。
     *
     * 前置条件：楼层必须已通关（floor <= towerBestFloor），消耗战票，产出经验+好感。
     * 连胜奖励：连续扫荡多层时，每3层额外+50%经验奖励（鼓励一次性清票）。
     */
        ...
        return core.withWriteLock {
            ...
            val outcome = core.transactionLocked(
                ...
                mutate = {
                    ...
                    core.appendBattleRecordCapped(
                        BattleRecord(
                            enemyName = "无尽之塔·第${floor}层（扫荡×${times}）",
                            enemyElement = "",
                            victory = true,
                            turns = 0,
                            remainingHp = 100,
            ...
            when (outcome) {
                ...
                WriteOutcome.SaveFailed -> TowerSweepOutcome.SaveFailed
            }
        }
    }
    ...
    /** 扫荡连扫信息（契约名 [TowerApi.getTowerSweepStreakInfo]）。 */
    @Deprecated("P2-11: 扫荡功能UI零调用", level = DeprecationLevel.WARNING)
    override fun getTowerSweepStreakInfo(times: Int): TowerSweepStreakInfo {
        val streakBonusMultiplier = 1.0 + ((times - 1) / 3) * 0.5
        val streakTier = (times - 1) / 3
        return TowerSweepStreakInfo(
            times = times,
            streakTier = streakTier,
            bonusMultiplier = streakBonusMultiplier,
            nextBonusAt = (streakTier + 1) * 3 + 1,
        )
    }
    ...
    override fun getTowerStats(): TowerStats {
        val bestFloor = saveData.towerBestFloor
        return TowerStats(
            bestFloor = bestFloor,
            maxFloor = EconomyFormulas.towerMaxFloor(),
            progressPercent = if (EconomyFormulas.towerMaxFloor() > 0) {
                bestFloor.toFloat() / EconomyFormulas.towerMaxFloor()
            } else 0f,
            ticketsRemaining = core.itemCount(ServiceCore.BattleTicketItemId),
        )
    }
}

/** 爬塔扫荡结果。 */
sealed interface TowerSweepOutcome {
    data class Success(
        val floor: Int,
        val times: Int,
        val totalExp: Int,
    ...
    data object Rejected : TowerSweepOutcome
    data object SaveFailed : TowerSweepOutcome
}

/** 连胜扫荡奖励信息。 */
data class TowerSweepStreakInfo(
    val times: Int,
    val streakTier: Int,
