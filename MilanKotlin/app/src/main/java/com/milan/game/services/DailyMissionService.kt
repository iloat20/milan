 ...
 * - 跨日重置
 */
internal class DailyMissionService(
     ...
     * getDailyMissionData）都会触发。重置已迁至 [ensureTodayReset]（事务 + 落盘）。
     */
    fun getData(): DailyMissionSaveData {
     ...
     *
     * R5-I4：跨日重置从只读 [getData] 迁入本 suspend 事务，原子改内存 + 落盘 + 失败回滚。
     * 由写路径（[reportProgress]/[claimActivityChest]）与进入每日任务页时调用。
     */
     ...
     * 进度已推进且永不回滚，并与临界区写并发竞争。现改为 suspend 事务：先 [ensureTodayReset]
     * 跨日重置，再在同一事务内推进进度并落盘，失败回滚。
     */
    suspend fun reportProgress(type: DailyMissionType, amount: Int = 1) {
        if (amount <= 0) return
        ensureTodayReset()
        val data = getData()
    ...
    /** 累计完成任务数（成就判定）。 */
    fun getTotalCompleted(): Int = getData().totalMissionsCompleted
}
