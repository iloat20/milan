 ...
 * - 重置机制
 */
internal class EventRhythmService(
    private val core: ServiceCore,
    private val rng: Random = Random.Default,
) {

    /** 获取活动数据。 */
    fun getData(): EventRhythmSaveData {
    ...
    fun getActiveEvents(): List<GameEvent> {
        ...
        return getData().activeEvents.filterNotNull().filter { event ->
    ...
    fun isEventActive(eventId: String): Boolean {
        val event = getData().activeEvents.firstOrNull { it?.eventId == eventId } ?: return false
        val now = System.currentTimeMillis()
    ...
    /** 更新任务进度。 */
    suspend fun updateTaskProgress(eventId: String, taskId: String, progress: Int): WriteOutcome {
        val data = getData()
    ...
    /** 领取任务奖励。 */
    suspend fun claimTaskReward(eventId: String, taskId: String): WriteOutcome {
        val data = getData()
        val event = data.activeEvents.firstOrNull { it?.eventId == eventId }
            ?: return WriteOutcome.Rejected
        val task = event.tasks?.firstOrNull { it?.taskId == taskId }
        ...
        if (currentProgress < (task.target ?: 1)) return WriteOutcome.Rejected
        // 已领取门控：防无限重复领取（永动机）
    ...
    /** 兑换活动商店物品。 */
    suspend fun redeemShopItem(eventId: String, itemId: String, amount: Int = 1): WriteOutcome {
        val data = getData()
        val event = data.activeEvents.firstOrNull { it?.eventId == eventId }
            ?: return WriteOutcome.Rejected
        ...
        if (currentRedemptions + amount > (shopItem.maxRedemptions ?: EventRhythmSaveData.MAX_SHOP_REDEMPTION_PER_ITEM)) {
            return WriteOutcome.Rejected
        }

        // Long 中介防 Int 溢出翻负
        val totalCost = (shopItem.price ?: 0).toLong() * amount
    ...
    /** 获取活动商店物品列表。 */
    fun getShopItems(eventId: String): List<EventShopItem> {
        val event = getData().activeEvents.firstOrNull { it?.eventId == eventId }
            ?: return emptyList()
        return event.shopItems?.filterNotNull() ?: emptyList()
    }
    ...
    suspend fun signIn(eventId: String): WriteOutcome {
        ...
        if (currentDays >= (event.signInDays ?: 0)) return WriteOutcome.Rejected
    ...
    fun getSignInProgress(eventId: String): Int {
        return getData().signInProgress[eventId] ?: 0
    }
    ...
    fun getEventCurrencyBalance(currencyType: String): Int {
        return getData().eventCurrencyBalances[currencyType] ?: 0
    }
