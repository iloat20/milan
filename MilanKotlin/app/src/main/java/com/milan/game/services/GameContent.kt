...
/**
 * GameService 的内置兜底内容（C# GameContent.cs 翻译）。
 *
 * 为什么独立成文件：这段角色/剧情/语音/武器表在 C# 侧有 420 行硬编码字面量，
 * 直接压在 GameService 主体里会把业务逻辑（抽卡事务、养成事务、存档回滚）淹没在数据中间，
...
internal object GameContent {
    ...
    /**
     * 为每名角色构建天赋树（3 分支 × 6 节点）。
     * 对齐 C# BuildTree/NewTalent：NodeId 与前置节点都带角色前缀，
     * VisualLayerId = 分支 id。
     */
    fun buildTalentTrees(characters: List<CharacterDataEntry>): List<TalentTreeData> =
        characters.map { ch ->
            val branches = listOf("branch_power", "branch_defense", "branch_utility")
            val nodes = listOf(
            ...
            TalentTreeData(treeId = ch.talentTreeId, branchIds = branches, nodes = nodes)
        }

    private fun newTalent(
        ch: CharacterDataEntry, id: String, name: String, branch: String,
        cost: Int, desc: String, prereq: String? = null,
        ...
        branchId = branch,
        cost = cost,
        prerequisiteNodeIds = if (prereq != null) listOf(ch.characterId + "_" + prereq) else emptyList(),
     ...
     * 两条加载路径（data.json / 兜底）都必须调用，保证字段口径一致（C# #31）。
    ...
    fun enrich(characters: List<CharacterDataEntry>) {
        ...
        val ambientVfx: Map<String, String> = mapOf(
            "char_ur_zhulong" to "day_night_cycle_glow", "char_ur_wuxu" to "void_devour_particles", "char_ur_xingtian" to "scrap_storm_ironveil",
            "char_ur_kikyo" to "soul_petal_drift", "char_ur_keqing" to "thundercloud_city", "char_ssr_fenghuang" to "ember_rebirth_field",
            "char_ssr_xiangliu" to "poison_marsh_fog", "char_ssr_leishen" to "electric_coil_core", "char_ssr_feilian" to "wind_tunnel_speedlines",
            "char_ssr_shangyang" to "constellation_guidance", "char_sr_suanni" to "pride_aura_flame", "char_sr_jingwei" to "sea_stone_mist",
            "char_sr_qiongqi" to "blood_metal_smoke", "char_sr_xuanwu" to "jade_shield_earth", "char_sr_bifang" to "storm_feather_spark",
            "char_r_lili" to "dust_earth_burst", "char_r_qinyuan" to "mechanical_bee_cloud", "char_r_sishu" to "night_city_shadows",
            "char_r_luoyu" to "underwater_bubble_light", "char_r_dangang" to "harvest_wind_leaves", "char_ur_jinwu" to "broken_suns_inferno",
        ...
        val weaponName: Map<String, String> = mapOf(
        ...
        for (c in characters) {
            ...
            if (c.weapon.isEmpty()) c.weapon = weaponName[c.characterId] ?: ""
            if (c.weaponDesc.isEmpty()) c.weaponDesc = weaponDesc[c.characterId] ?: ""
        }
    }
}
