 ...
 */
internal class GachaService(private val core: ServiceCore) {
    ...
    suspend fun pull(poolId: String, tenPull: Boolean): PullOutcome = core.withWriteLock {
        ...
        for (i in 0 until count) {
            ...
            // minRarityForPity 直接传 Rarity 类型（SSR），类型化后不再有 value/下标歧义。
            val rolledRarity = pity.rollWithPity(core.rng, pool.rarityWeights.toIntArray(), Rarity.SSR).value
