...
/**
 * 简化版伤害飘字（单条事件，用于非战斗场景的即时反馈）。
 */
@Composable
fun SingleDamageFloat(
    damage: Int,
    isCrit: Boolean,
    isDefeated: Boolean,
    modifier: Modifier = Modifier,
    ...
    Canvas(modifier = modifier) {
        ...
        val alpha = when {
            ...
            progress > 0.5f -> 1f - (progress - 0.5f) / 0.5f
            else -> 1f
        }.coerceIn(0f, 1f)

        val color = when {
            isDefeated -> AppTheme.SealRed
            isCrit -> AppTheme.Gold
            else -> AppTheme.Text1
        }

        val text = if (damage == 0) "MISS" else "-$damage${if (isDefeated) " †" else ""}"

        drawDamageText(
            text = text,
            x = size.width / 2f,
            y = size.height / 2f - progress * 80f,
            scale = 1f,
            alpha = alpha,
            color = color,
            textMeasurer = textMeasurer,
