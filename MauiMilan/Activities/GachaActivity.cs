...
public class GachaActivity : Activity
{
    ...
    {
        base.OnResume();
        // Only refresh currency, do NOT rebuild layout (would erase results).
    ...
    LinearLayout BuildLayout()
    {
        ...
        top.AddView(back); top.AddView(headTitle); top.AddView(spacer); top.AddView(_currency);
        root.AddView(top);
        root.AddView(Spacer(18));

        var banner = new LinearLayout(this) { Orientation = Orientation.Vertical };
        ...
        var poolHint = UI.Text("UR 10%  ·  SSR 20%  ·  SR 30%", 12, AppTheme.TextSecondary);
        poolHint.SetPadding(0, Dp(4), 0, 0);
        banner.AddView(poolName); banner.AddView(poolHint);
        root.AddView(banner);
        root.AddView(Spacer(18));
