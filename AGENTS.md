...
- **立绘美术方案以 `docs/superpowers/art-direction/portraits-v2/` 为准（v2.0，2026-08-13）**：`00-master-spec.md` 总规范 + `01~04-character-portrait-*.md` 四档角色稿（UR/SSR/SR/R 各 7 位，共 28）+ `05-quality-baseline.md` 现状基线。`character-designs-*.md` 与 `visual-redesign-spec.md` 为 MAUI 时代 v1.0 旧稿，已废弃，勿按生图。`portraits-v2/extract_prompts.py` 解析稿为结构化 prompt 导出 `prompts-export.json`；`generate_portraits.py` 可调用生图 API（mock/openai/flux）批量产出 `char_<rarity>_<拼音>.webp`（透明背景、阶梯画布，与 `PortraitLoader` 命名对齐）。
