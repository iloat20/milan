# Milan 设计语言 v3.1 ——「织环 · Ringweave」

> 版本：v3.1（2026-09-12）　状态：**概念已定（产品名「织环」）**
> 范围：全局视觉设计语言 + 设计系统 + 命名栈。适配工程现状：Kotlin / Jetpack Compose / Material3（BOM 2026.06.01）/ AGSL 特效层（API 33+ 判定）。
...
> v3.0「丹青典藏 · Gilded Codex」已于 2026-09-12 **概念层废弃**（美术 token / 工艺稀有度 / 组件骨架继承，仅换品牌隐喻与主张句）。依据：`research/milan-theme-fit/REPORT.md`。

## 0. 文档定位与废弃清单

本文档是设计语言的**单一事实来源**，取代以下互相矛盾的旧规范（内容仅作历史参考，勿再按其执行）：
...
| 代码注释「云海仙气·丹青录 v2」 / v3.0「丹青典藏馆」 | **概念已换芯** | 品牌隐喻改为「织环」；水墨降为 Shinwa L2 |
...
| B. 软糖亲和（Supercell 系） | 圆润软 3D | ❌ 与 v2.3 写实卡面、暗色基调冲突，受众错位 |
| C. 丹青典藏（v3.0） | "一座只陈列珍本的典藏馆" | ❌ **概念废弃**：场所名词+被动「观看」动词，与裂隙重聚叙事、写实工艺卡面、三界异质同时错位（见 research 报告） |
| **D. 织环（v3.1 采纳）** | **"裂隙打碎原初之环，你是织环者——召英灵、聚环痕、重铸世界"** | ✅ 行动动词 + 可数名词 + 三界可作 L2；继承 v2/v3 全部美术与工艺 token |

### 3.3 设计语言：「织环 · Ringweave」

**概念主张（一句话）**：裂隙撕开三界，**你是织环者**——在碎片间召唤英灵，把破碎的原初之环重新拼回世界。界面是**织环台**：卡牌是待归位的环痕，按钮与面板是梭与纬线，玩家不是馆长，是行动者。

**命名栈（对外/系统词）**：

| 层 | 词 | 说明 |
| --- | --- | --- |
| L0 产品名 | **织环** | 行动向短名；英文 Ringweave |
| 玩家身份 | **织环者** | 与剧情主轴同动词 |
| 角色本体 | **英灵** | 沿用中文强词；域外角色写「环痕共鸣者 / 异界访客」，不作 IP 联动注记 |
| 物质/货币 | **环痕 · 残玦 · 纯环** | soft=环痕 / hard=纯环 / starFragments=残玦；UI 显示名见 `ui/theme/CurrencyNames.kt`（不改存档字段） |
| 集合机构 | **织环台** | 行动枢纽（对标列车/Chaldea），**不是展厅** |
| 区域 | Shinwa 神話 / Aether 以太 / Ironveil 铁帷 | L2，允许视觉完全异质 |
| 水墨/朱印/绫绢 | **仅 Shinwa 阵营氛围 + 限时皮肤线** | 全站 chrome 用中性玄墨 + 金箔 + 材质光 |

**范式转变**：
- v2：水墨画在界面上（装饰皮肤）
- v3.0：界面向卡牌供奉，但隐喻是**静态典藏馆**（错误）
- **v3.1：界面向重聚供奉**——一切元素服务「召 / 织 / 归位」；水墨从全站品牌降为 **Shinwa L2 与可售卖皮肤线**（对标王者墨染江湖：文化是内容池与皮肤，不是产品名）。

**六条设计原则（每条可判定，评审时逐条对照）：**
1. **卡牌即主角**：任何界面只有一个视觉焦点对象；面板、按钮、文字都是织环台的梭架与射灯。
2. **材质代替装饰**：深度/状态/层级用材质（玻璃、箔、丝线、光）表达，不叠装饰线；删掉一切"为了好看"的第三层描边。
...
6. **动有重量，一次一动**：动效有物理感（弹簧、惯性、落纬的"顿"）；同一时刻只有一个主对象做 emphasized 动画。抽卡仪式可保留朱印/墨迹作为**仪式瞬间**，不作全局 chrome。
...
| 角色 | 字体 | 说明 |
| --- | --- | --- |
| 品牌 + 仪式标题 | 马善政楷书 | **仅 2 处**：「织环」Logo、抽卡仪式标题。不再作 headlineSmall |
...
### 7.1 主页 = 织环台
- **焦点对象**：Hero 即环台——立绘居中 + 同心环 + 纬向弧；世界/稀有度 chip 在顶；铭牌 + 主 CTA「召灵」**叠在环台内底部**（不再单独占一行按钮）。
- 快捷入口改「纬带」横滑 chip；环痕名录横滑保留。
- 环台随滚动轻微视差；世界氛围只染背景。
...
### 7.4 战斗/爬塔 = 对弈台（独立 HUD，可读性优先）
- **独立主题 `ui/battle/BattleTheme.kt`**：与菜单织环台分离——Stage 深底、Ally 青 / Enemy 红 / Focus 黄，禁玻璃与装饰金环。
- 不用 `PageBackground`（环水印）；战术顶栏 + 敌我标签条 + 实底战报 + 高对比确认键。
- HP 条：绿→黄→红三档；能量条 Ally 青。§5.7 对比度预算生效。
- 战力预览条（石青 vs 朱砂）保留在塔页；结算仍可走 BattleResultOverlay。
